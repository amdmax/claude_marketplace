---
name: sync-skills
description: Push local skills to https://github.com/amdmax/agent_skills for reuse across projects. Discovers skills, syncs to remote repository, generates index, and reports results. Use when you want to share skills across projects or when user says "sync skills", "push skills to GitHub", or "share skills".
---
...
3. **Syncs to GitHub** - Clones/pulls agent_skills repo, copies skills
...
- **Write access** to target repository (https://github.com/amdmax/agent_skills)
...
**Expected output:**
```
✓ Logged in to github.com as amdmax (keyring)
...
sync-skills                 |        |         | 1     | Push skills to agent_skills repo
...
```
...
**Clone agent_skills repository:**
```bash
# Clone using gh CLI (uses existing auth)
gh repo clone amdmax/agent_skills
...
gh repo create amdmax/agent_skills --public --description "Shared Claude Code skills for cross-project reuse"
...
for SKILL_NAME in "${SELECTED_SKILLS[@]}"; do
  ...
  # Create skill directory in agent_skills repo
  mkdir -p "$SYNC_DIR/agent_skills/$SKILL_NAME"

  # Copy entire skill directory structure
  rsync -av --exclude='*.tmp' --exclude='.DS_Store' \
    "$PROJECT_ROOT/.claude/skills/$SKILL_NAME/" \
    "$SYNC_DIR/agent_skills/$SKILL_NAME/"

  # Verify copy
  if [ -f "$SYNC_DIR/agent_skills/$SKILL_NAME/SKILL.md" ]; then
...
git submodule add https://github.com/amdmax/agent_skills.git .claude/agent_skills

# Symlink skills you want to use
ln -s .claude/agent_skills/gh:commit .claude/skills/gh:commit
ln -s .claude/agent_skills/mr .claude/skills/mr
```
...
**If no changes:**
```
No changes detected. All skills are already up-to-date in agent_skills repo.
```
...
2. No write access to amdmax/agent_skills
3. Branch protection rules require PR

Solutions:
- Run: gh auth refresh
- Check: gh repo view amdmax/agent_skills
...
Synced Skills:
✓ commit      → https://github.com/amdmax/agent_skills/tree/main/gh:commit
...
1. Clone: gh repo clone amdmax/agent_skills
2. Copy: cp -r agent_skills/<skill-name> .claude/skills/
...
2. Check repository access: gh repo view amdmax/agent_skills
...
if [ ! -d "agent_skills/$SKILL_NAME" ]; then
  echo "✗ Skill '$SKILL_NAME' not found in agent_skills repo"
...
```bash
rsync -av "agent_skills/$SKILL_NAME/" ".claude/skills/$SKILL_NAME/"
```
...
```bash
# Check repository
gh repo view amdmax/agent_skills

# If doesn't exist, create it
gh repo create amdmax/agent_skills --public
...
1. **Sync regularly** - Keep agent_skills repo up-to-date with improvements
...
- **Private repositories:** Ensure agent_skills repo is private if syncing proprietary skills
- **Access control:** Verify who has write access to agent_skills repository
...
- [ ] Repository exists: `gh repo view amdmax/agent_skills`
- [ ] Write access: `gh repo view amdmax/agent_skills --json permissions`
