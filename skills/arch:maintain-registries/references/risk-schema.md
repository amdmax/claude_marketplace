...
- Examples: "No automated rollback for Lambda@Edge deployments", "Cognito free tier limits"

**Story-level risks** (`scope: story`, `storyRef: AIGWS-NNN`):
- Risks specific to one story's implementation
- Written by architect during design (Step 6 of architect workflow)
...
To find the next ID:
```bash
grep '  - id: RISK-' docs/risks.yaml | tail -1 | grep -o '[0-9]*$'
# Increment that number by 1 and zero-pad to 3 digits
```

## Bootstrap Template

If `docs/risks.yaml` does not exist, create it with:

```yaml
# Project Risk Registry
# Maintained by: architect agent + human review
# Updated: on each story design (Step 6 of architect workflow)
