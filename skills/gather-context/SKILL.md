...
3. **Reviews architecture docs** (`_bmad-output/`) for architectural decisions
4. **Checks existing ADRs** (`docs/adr/`) for precedents
5. **Asks user questions** for dependencies, constraints, and preferences
6. **Appends context** to `.agile-dev-team/technical-context.json` for ADR generation
...
### 3. Architecture Documentation (`_bmad-output/`)
...
Glob: pattern="_bmad-output/**/*architecture*.md"
Glob: pattern="_bmad-output/**/*tech-spec*.md"

# Search for payment-related docs
Grep: pattern="payment|billing|checkout", path="_bmad-output/", output_mode="files_with_matches"

# Read relevant docs
Read: _bmad-output/payment-referral-architecture.md
Read: _bmad-output/phase3-tech-spec.md
```
...
Grep: pattern="payment|stripe|checkout|billing", path="docs/adr/", output_mode="files_with_matches"

# Read relevant ADRs
Read: docs/adr/0001-stripe-payment-processor.md
Read: docs/adr/0005-lambda-edge-auth.md
```
...
if [ ! -f "$STORY_FILE" ]; then
  ...
  echo "   Expected: $CLAUDE_PROJECT_DIR/.agile-dev-team/active-story.json"
...
**Check NFRs:**
```javascript
const story = JSON.parse(fs.readFileSync('.agile-dev-team/active-story.json', 'utf-8'));
let techContext = {};
try { techContext = JSON.parse(fs.readFileSync('.agile-dev-team/technical-context.json', 'utf-8')); } catch {}

if (!techContext.nfrs || Object.keys(techContext.nfrs).length === 0) {
...
# Search by keywords
Grep: pattern="payment|checkout", path="_bmad-output/", output_mode="files_with_matches"
```
...
```json
{
  "relatedArchitecture": [
    {
      "path": "_bmad-output/payment-referral-architecture.md",
      "relevantSections": [
...
Grep: pattern="payment|stripe|checkout", path="docs/adr/", output_mode="files_with_matches"
```
...
```json
{
  "relatedADRs": [
    {
      "path": "docs/adr/0001-stripe-payment-processor.md",
    ...
    {
      "path": "docs/adr/0005-lambda-edge-auth.md",
...
### Step 9: Write Context to technical-context.json

**Read or initialize technical-context:**
```javascript
let techContext = {};
try { techContext = JSON.parse(fs.readFileSync('.agile-dev-team/technical-context.json', 'utf-8')); } catch {}
```
...
**Write updated technical-context:**
```javascript
...
Architecture:
  • _bmad-output/payment-referral-architecture.md - Payment processing design
...
✓ Context saved to .agile-dev-team/technical-context.json
...
Searched: docs/adr/ for keywords [payment, checkout, stripe]
