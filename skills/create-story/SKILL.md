---
description: Quick GitHub issue creation for commit workflow. Creates minimal issues with AIGCODE prefix and stores in .agile-dev-team/active-story.json. Different from /fetch-story (planned work from Projects).
...
3. **Stores minimal data** in `.agile-dev-team/active-story.json` (4 fields only)
...
> **⚠️ IMPORTANT:** `gh issue create` does NOT support `--json` flag (unlike `gh issue view`).
> Capture the URL from stdout and parse it with grep instead.
...
# Note: gh issue create outputs the issue URL to stdout (NOT JSON)
...
echo "✓ Active story saved: .agile-dev-team/active-story.json"
echo "$ISSUE_URL"
```
