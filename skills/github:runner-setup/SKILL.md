...
**Parameters:**
- `org-or-repo`: Organization name (e.g., `{{REPO_OWNER}}`) or repository path (e.g., `{{REPO_OWNER}}/website`)
...
```bash
./scripts/install_runner.sh {{REPO_OWNER}} macos-prod-runner-01 "macos-arm64,node-22,aws-deployment,claude-code"
```

**Example - Repository Runner:**
```bash
./scripts/install_runner.sh {{REPO_OWNER}}/website macos-repo-runner-01 "macos-arm64,node-20"
```
...
jobs:
  deploy:
    runs-on: [self-hosted, macos-arm64, node-22, aws-deployment, {{REPO_OWNER}}-prod]
