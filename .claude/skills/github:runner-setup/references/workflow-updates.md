...
git commit -m "{{PROJECT_PREFIX}}-XXX: Test self-hosted runner for <workflow-name>"
...
git add .github/workflows/<workflow-file>.yml
git commit -m "{{PROJECT_PREFIX}}-ROLLBACK: Revert <workflow-name> to GitHub-hosted runner"
...
```yaml
runs-on: ${{ github.event_name == 'push' && github.ref == 'refs/heads/master' && '[self-hosted, macos-arm64, {{REPO_OWNER}}-prod]' || 'ubuntu-latest' }}
```
