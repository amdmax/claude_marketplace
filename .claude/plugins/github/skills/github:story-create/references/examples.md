...
# → ✓ Issue #158 created: https://github.com/{{REPO_SLUG}}/issues/158
# → ✓ Active story saved

# Commit continues with issue number
# → {{PROJECT_PREFIX}}-158: Fix token expiration check in auth lambda
```

**Result**:
- GitHub issue #158 created
- `.claude/active-story.json` contains minimal data (4 fields)
...
# → ✓ Active story saved: .claude/active-story.json
...
# → {{PROJECT_PREFIX}}-159: Add Chart.js wrapper with theme support
```
...
```json
{
  ...
  "url": "https://github.com/{{REPO_SLUG}}/issues/160"
}
```
...
/commit
# → {{PROJECT_PREFIX}}-162: Fix typo in introduction section
...
# → {{PROJECT_PREFIX}}-157: Add LSP architecture overview
```
...
# → {{PROJECT_PREFIX}}-124: Add tests for authentication module
...
# → ✓ Created issue #163
# → Note: Link commit {{PROJECT_PREFIX}}-124 in PR description
```
