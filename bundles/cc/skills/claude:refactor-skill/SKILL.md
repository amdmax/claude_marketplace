...
120:## Step 2: Find Next {{PROJECT_PREFIX}} Number
...
grep -nE "[A-Z]{2,10}-[0-9]+" SKILL.md  # Like {{PROJECT_PREFIX}}-###
...
Conventions:
- commit_prefix: "{{PROJECT_PREFIX}}" (8 occurrences)
...
**Conventions:**
- `conventions.commit_prefix`: Commit number prefix (default: {{PROJECT_PREFIX}})
...
```markdown
# {{PROJECT_PREFIX}} Numbering System

> **Reference for:** {skill-name}
> **Context:** This document is loaded when detailed {{PROJECT_PREFIX}} numbering logic is needed.
...
```markdown
## Step 2: Find Next {{PROJECT_PREFIX}} Number

The skill uses sequential numbering across all branches.

**Quick usage:**
```bash
git log --oneline --all --grep="{{PROJECT_PREFIX}}-" | grep -o "{{PROJECT_PREFIX}}-[0-9]*" | sort -u | tail -1
```

**For detailed algorithm and edge cases:** See [{{PROJECT_PREFIX}} Numbering](references/aigcode-numbering.md)
```
