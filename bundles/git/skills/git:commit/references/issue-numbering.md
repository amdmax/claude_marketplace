...
```bash
# Read ${AGENT_DOCS_DIR:-docs}/active-story.yaml
...
If `${AGENT_DOCS_DIR:-docs}/active-story.yaml` doesn't exist:
...
4. Minimal data written to `${AGENT_DOCS_DIR:-docs}/active-story.yaml` (issueNumber, title, body, url)
...
❌ **Don't manually edit** `${AGENT_DOCS_DIR:-docs}/active-story.yaml` - use skills
