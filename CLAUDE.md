...
| `agile-team` | `agent:architect`, `agent:backend-dev`, `agent:devops`, `agent:frontend-dev`, `agent:test-architect`, `pm`, `pm:build`, `scout:gather-context`, `scout:prepare-for-dev`, `team:agile-dev` | Agile dev team — full 5-agent TDD team or individual agents for controlled, single-agent delegation; `/pm build` orchestrates a fleet of specialist agents driven by the architect's implementation plan |
