...
| `architecture-quality` | `arch:adr-render`, `arch:adr-yaml`, `arch:create-adr`, `arch:fitness-function`, `arch:maintain-constraints-registry`, `arch:maintain-nfr-registry`, `arch:maintain-risk-registry`, `arch:mermaid-diagram`, `review:code`, `review:compliance`, `review:overall`, `review:performance`, `review:security` | Architecture guidance and code quality — ADRs, reviews, fitness functions, NFR/risk/constraints registries, diagrams |
| `aws` | `aws:architect`, `aws:cdk`, `aws:cdk-validate` | AWS infrastructure — architect guidance, CDK, CDK validation |
| `content-specialized` | `add-content-image`, `creative-writing`, `css-architecture`, `cuda-remote-manager`, `design-system`, `editor-in-chief`, `gather-nfr`, `regenerate-course-content`, `reveal-pdf-export`, `ux-professional` | Content creation and specialized tools — writing, CSS, UX, design systems, documentation |
