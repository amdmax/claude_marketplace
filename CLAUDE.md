...
| `github` | `github:actions`, `github:issue-create`, `github:edit-workflow`, `github:merge-chores`, `github:pull-request`, `github:runner-setup`, `github:story-create`, `github:story-fetch`, `github:story-finalize`, `github:story-play`, `github:story-quality`, `github:tidy-board`, `github:until-green`, `scout:prepare-for-dev` | GitHub-hosted workflow — issues, PRs, CI loops, story lifecycle, board management, runner setup, and sequential chore merging |
| `development-tools` | `bug-fix`, `debug`, `experimentator`, `scout:gather-context`, `jupyter-remote` | Developer utilities — debugging, context gathering, experimentation, remote notebooks |
...
| `architect` | `arch:adr-render`, `arch:adr-yaml`, `arch:create-adr`, `arch:design-implementation`, `arch:fitness-function`, `arch:gather-nfr`, `arch:maintain-constraints-registry`, `arch:maintain-nfr-registry`, `arch:maintain-risk-registry`, `arch:mermaid-diagram`, `review:code`, `review:compliance`, `review:overall`, `review:performance`, `review:security` | Architecture guidance and code quality — ADRs, story design lifecycle, NFR gathering, reviews, fitness functions, registries, diagrams |
